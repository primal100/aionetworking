...
async def run_until_signal(conf: Union[str, Path], paths: Dict[str, Union[str, Path]] = None):
    manager = SignalServerManager(conf, paths=paths)
...
def run_forever_in_loop(conf_file, paths=None, asyncio_debug: bool = False):
    load_all_tags()
...
def run_server(conf_file, paths: Dict[str, Union[str, Path]] = None, asyncio_debug: bool = False):
    if os.name == 'posix':
        asyncio.run(run_until_signal(conf_file, paths=paths), debug=asyncio_debug)
    else:
        try:
            asyncio.run(run_forever(conf_file, paths=paths), debug=asyncio_debug)
...
def run_server_default_tags(conf_file, paths: Dict[str, Union[str, Path]] = None, asyncio_debug: bool = False):
    load_all_tags()
    run_server(conf_file, paths=paths, asyncio_debug=asyncio_debug)
