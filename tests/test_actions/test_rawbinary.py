...
from .base import BaseTCAPMAPRawActionTestCase
from lib.actions import rawbinary


class TestBinaryActionTCAPMAP(BaseTCAPMAPRawActionTestCase):
    log_level = logging.DEBUG
    action_cls = rawbinary.Action

    def test_00_content(self):
        content = self.action.get_content(self.packet)
    ...
    def test_01_print_msg(self):
        result = self.print_action.print_msg(self.packet)
    ...
    def test_06_do(self):
        ...
        msgs = asyncio.run(self.protocol.from_file(expected_file, sender=self.sender))
        self.assertTupleEqual(msgs[0].decoded, ('begin', {'otid': b'\x00\x00\x00\x01', 'dialoguePortion': {
    ...
    def test_07_do_many(self):
        asyncio.run(self.do_async(self.action.do_multiple(self.msgs)))
        expected_file = Path(self.base_data_dir, 'Encoded', '10.10.10.10_TCAP_MAP.TCAPMAP')
        self.assertBinaryFileContentsEqual(expected_file,
