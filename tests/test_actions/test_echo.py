...
class TestEcho:
    ...
    @pytest.mark.asyncio
    async def test_01_do_notification(self, echo_action, echo_notification_request_object, echo_notification, peer):
        response = await echo_action.do_one(echo_notification_request_object)
        assert response is None
        notification = await aone(echo_action.get_notifications(peer))
