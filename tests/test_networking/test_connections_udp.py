...
class TestConnectionShared:
    ...
    @pytest.mark.asyncio
    async def test_00_connection_made_lost(self, udp_connection, udp_transport, udp_adaptor, connections_manager,
                                           udp_connection_is_stored):
        assert not udp_transport.is_closing()
        assert connections_manager.total == 0
        assert udp_connection.logger
        assert udp_connection.transport is None
        udp_connection.connection_made(udp_transport)
        await udp_connection.wait_connected()
        assert udp_connection.is_connected()
        assert not udp_transport.is_closing()
        assert udp_connection._adaptor.context == udp_adaptor.context
        assert udp_connection._adaptor == udp_adaptor
        assert udp_connection.peer == f"tcp_{udp_adaptor.context['peer']}"
        assert udp_connection.logger is not None
        assert udp_connection.transport == udp_transport
        total_connections = 1 if udp_connection_is_stored else 0
        assert connections_manager.total == total_connections
        if udp_connection_is_stored:
            assert connections_manager.get(udp_connection.peer) == udp_connection
        udp_connection.connection_lost(None)
        assert udp_transport.is_closing()
        await udp_connection.wait_closed()
        assert connections_manager.total == 0
