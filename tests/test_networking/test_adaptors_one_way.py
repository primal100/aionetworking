...
class TestOneWayReceiverAdaptor:
    @pytest.mark.asyncio
    async def test_00_on_data_received(self, tmp_path, one_way_receiver_adaptor, json_buffer,
                                       json_rpc_login_request_encoded, json_rpc_logout_request_encoded,
                                       timestamp, json_recording_data, json_codec, buffer_codec, json_objects,
...
class TestSenderAdaptorOneWay:
    ...
    @pytest.mark.asyncio
    async def test_00_close(self, one_way_receiver_adaptor):
        await one_way_receiver_adaptor.close()

    @pytest.mark.asyncio
    async def test_01_send(self, one_way_sender_adaptor, json_rpc_login_request_encoded, queue):
        one_way_sender_adaptor.send(json_rpc_login_request_encoded)
        assert queue.get_nowait() == json_rpc_login_request_encoded

    @pytest.mark.asyncio
    async def test_02_send_data(self, one_way_sender_adaptor, json_rpc_login_request_encoded, queue):
