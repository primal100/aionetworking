...
class TestSenderInExecutors:
    ...
    async def test_00_send_in_loop_in_other_thread(self, connections_manager, receiver_debug_logging_extended,
                                                   ...
                                                   tmp_path, json_encoded_multi, json_decoded_multi,
                                                   recording_file_name, json_buffer):
        msgs = []
        num = 1000
        for i in range(0, num):
            msgs += json_encoded_multi
        ...
        assert len(expected_file.read_bytes()) == len(json_buffer) * num
