...
class TestOneWayServer:
    ...
    async def test_00_send(self, server_started, client_connection_started, json_decoded_multi,
                           assert_server_buffered_file_storage_ok, fixed_timestamp):
    ...
    @pytest.mark.asyncio
    async def test_01_send_recording(self, server_started, client_connection_started, json_decoded_multi,
                                     assert_server_buffered_file_storage_ok, assert_recordings_ok,
                                     expected_recordings_file, fixed_timestamp):
        client_connection_started.encode_and_send_msgs(json_decoded_multi)
        await asyncio.wait_for(server_started.wait_num_has_connected(1), 3)
        await asyncio.sleep(0.1)
        server_started.close_all_connections()
        await server_started.wait_num_connections(0)
        await asyncio.wait_for(server_started.wait_all_tasks_done(), 3)
        await assert_recordings_ok
        client_connection_started.play_recording(expected_recordings_file)
        await asyncio.sleep(0.1)
        await asyncio.wait_for(server_started.wait_all_tasks_done(), 3)
        await assert_server_buffered_file_storage_ok


@pytest.mark.connections('all_twoway_all')
class TestTwoWayServer:
    @pytest.mark.asyncio
    async def test_00_send(self, server_started, client, echo_response_object, echo_exception_response_object,
                           echo_notification_object, fixed_timestamp):
