...
class TestSSL:
    ...
    @pytest.mark.asyncio
    async def test_00_get_ssl_context_server(self, ssl_context):
        context = ssl_context.context
        assert context.check_hostname is False
        assert context.verify_mode == ssl.CERT_REQUIRED

    @pytest.mark.connections('tcp_oneway_client')
    @pytest.mark.asyncio
    async def test_00_get_ssl_context_server(self, ssl_context):
        context = ssl_context.context
    ...
    def test_02_no_ssl(self, server_side_no_ssl):
        context = server_side_no_ssl.context
        assert context is None

    @pytest.mark.asyncio
    async def test_03_cert_about_to_expire(self, server_side_ssl_short_validity, short_validity_cert_actual_expiry_time,
                                           caplog):
