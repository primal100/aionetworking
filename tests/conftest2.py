...
@pytest.fixture
def asn_objects(asn_encoded_multi, asn_decoded_multi):
    return [TCAPMAPASNObject(encoded, asn_decoded_multi[i], context={'sender': '127.0.0.1'},
                            received_timestamp=datetime(2019, 1, 1, 1, 1)) for i, encoded in
...
def buffered_file_storage_action(tmp_path):
    return BufferedFileStorage(base_path=tmp_path, path='Encoded/{msg.protocol_name}')
...
def file_containing_json(tmpdir, json_one_encoded):
    p = Path(tmpdir/'encoded/testjson')
    p.write(json_one_encoded)
    return p
...
@pytest.fixture
def json_object(json_one_encoded, json_rpc_request):
    return JSONObject(json_one_encoded, json_rpc_request, context={'sender': '127.0.0.1'})


@pytest.fixture
def json_objects(json_encoded_multi, json_decoded_multi):
...
def tcp_protocol(json_rpc_action):
    return TCPServerProtocol(JSONObject, action=json_rpc_action, timeout=4, aliases={'127.0.0.1': 'localhost'})


@pytest.fixture
def tcp_one_way_protocol(file_storage_action):
    return TCPOneWayServerProtocol(TCAPMAPASNObject, action=file_storage_action, aliases={'127.0.0.1': 'localhost', '127.0.0.2': 'localhost2'}, allowed_senders=('127.0.0.1', '127.0.0.2'))


@pytest.fixture
def udp_protocol(json_rpc_action):
    return UDPServerProtocol(JSONObject, action=json_rpc_action, aliases={'127.0.0.1': 'localhost'})


@pytest.fixture
def udp_one_way_protocol(buffered_file_storage_action):
    return UDPServerOneWayProtocol(TCAPMAPASNObject, action=buffered_file_storage_action, aliases={'127.0.0.1': 'localhost', '127.0.0.2': 'localhost2'}, allowed_senders=('127.0.0.1', '127.0.0.2'))
...
def json_rpc_requester():
    return SampleJSONRPCClient(timeout=3)
...
def tcp_client_protocol(json_rpc_requester):
    return TCPClientProtocol(JSONObject, action=json_rpc_action, timeout=4, aliases={'127.0.0.1': 'localhost'})


@pytest.fixture
def udp_client_protocol(file_storage_action):
    return UDPClientProtocol(JSONObject, action=file_storage_action, timeout=3, aliases={'127.0.0.1': 'localhost', '127.0.0.2': 'localhost2'})
